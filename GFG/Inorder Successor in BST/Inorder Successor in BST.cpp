...
class Solution{
  public:
    ...
    Node * inOrderSuccessor(Node *root, Node *x){
        ...
        Node *successor = NULL;    //initialize sucessor as null
        while(root != NULL)  //root have some value not empty
        {
            if(x->data >= root->data){   //root val smaller than equal to given val
                root = root->right;    //go to the right
            }
            else{                       // root val gretter than equal to given val
                successor = root;       // store root 
                root = root->left;       // check the other condition 
            }
        }
        return successor;
    }
